...
impl AppUI {
    ...
    fn populate_secret_names(self: &Self) {
        ...
        for section_name in secret_names {
            ...
            section_single.connect_expanded_notify({
                let cred_prov = Rc::clone(&self.credentials_provider);
                AppUI::populate_secrets(cred_prov)
            });
    ...
    fn populate_secrets(cred_prov: Rc<CredentialsProvider>) -> impl Fn(&Expander) {
        move |expander| {
            let secret_name = expander.widget_name();
            let secrets = cred_prov.load_secrets(secret_name.as_str()).expect(format!("Can't load a secret {}", secret_name.as_str()).as_str());
    ...
    fn build_labels(grid: &Grid, name: &str, value: &str, index: i32) {
    ...
    fn build_grid(section: &HashMap<String, String>) -> Grid {
        let grid = Grid::builder().column_spacing(25).row_spacing(5).build();
            ...
            if let Some(login) = section.get("login") {
                AppUI::build_labels(&grid, "login", login, 0);
                addon = addon + 1;
            }

            if let Some(password) = section.get("password") {
                AppUI::build_labels(&grid, "password", password, 1);
                addon = addon + 1;
            }
            ...
            for (index, (key, value)) in section.iter().enumerate() {
                ...
                AppUI::build_labels(&grid, key, value, (index as i32) + addon);
            }
        grid
    }
}
