...
use eframe::egui::{Align, CentralPanel, Id, Layout, Panel, TextEdit, Ui, Vec2, ViewportBuilder, ViewportId};
...
impl DeleteSecretUI {
    ...
    pub fn show(&mut self, ui: &mut Ui) {
        if self.open_dialog {
            ...
            ui.ctx().show_viewport_immediate(dialog_id, delete_secret_dialog, |ui, _| {
                if ui.ctx().input(|input_state| input_state.viewport().close_requested()) {
                    self.close();
                }

                Panel::bottom(Id::new("delete_bottom_panel")).show_inside(ui, |ui| {
                    ui.add_space(6.0);
                    ui.horizontal(|ui| {
    ...
    fn handle_delete(&mut self, ui: &mut Ui) {
        ...
        match self.credentials_provider.delete_secret(&self.secret_name) {
            Ok(_) => {
                Self::clear_ui_cache(ui, &self.secret_name);
                self.close();
            }
            Err(e) => {
    ...
    fn clear_ui_cache(ui: &mut Ui, secret_name: &str) {
